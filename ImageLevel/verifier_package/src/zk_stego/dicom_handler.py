...
Handles DICOM file loading, ROI detection, entropy analysis, and
two-key LSB steganographic embedding / extraction.
...
Embedding layout (pure LSB, no PNG chunk)
-----------------------------------------
proof_key region  (84-byte header + ZK proof bytes):
    [0:4]   b"ZKDC"                   magic
    [4:8]   version / flags            b"\\x01\\x00\\x00\\x00"
...
chaos_key region  (follows; non-overlapping with proof_key region):
...
class DicomStego:
    ...
    def embed(
        ...
        Bits 0 and 1 of each pixel are used (2 bits per pixel, no 8-bit normalisation).
        ...
        background_percentile : bottom N% of intensity excluded as background ROI
        ...
        _log(f"ROI    : {roi_count} / {pixel_array.size} pixels "
             f"({100 * roi_count / pixel_array.size:.1f} %)")
        ...
        if generate_zk_proof:
            ...
            # (prover computes image hash from raw bytes — shape doesn't matter)
        ...
        # 7. Capacity check  (2 bits per pixel → positions = ceil(bits / 2))
        needed_positions = _positions_needed(len(proof_bits)) + _positions_needed(len(meta_bits))
        if needed_positions > roi_count:
            raise ValueError(
                f"Insufficient ROI capacity: need {needed_positions} positions "
                f"but only {roi_count} ROI pixels available."
            )

        # 8. Proof-key positions  (no entropy sorting — fixed structure)
        ...
        proof_positions = self._roi_positions(
            ...
            _positions_needed(len(proof_bits)),
            sort_by_entropy=False,
            exclude=None,
        )

        # 9. Chaos-key positions  (entropy sorted, non-overlapping)
        proof_pos_set: Set[Tuple[int, int]] = set(map(tuple, proof_positions))
        meta_positions = self._roi_positions(
            pixel_array, roi_mask,
            meta_x0, meta_y0, chaos_key_int,
            _positions_needed(len(meta_bits)),
            sort_by_entropy=True,
            exclude=proof_pos_set,
        )
        _log(f"\nEmbedding proof block at {len(proof_positions)} positions ({len(proof_bits)} bits)")
        _log(f"Embedding metadata    at {len(meta_positions)} positions ({len(meta_bits)} bits)")

        # 10. LSB embedding
        stego = _embed_at(pixel_array.copy(), proof_positions, proof_bits)
    ...
    def extract(
        ...
        """
        Extract DICOM metadata from a stego PNG and verify the ZK proof.
        ...
        # ------------------------------------------------------------------
        # Step 3: read proof tail (proof JSON + public JSON)
        # ------------------------------------------------------------------
        proof_tail_len = header["proof_json_len"] + header["public_json_len"]
        total_proof_bits = (HEADER_SIZE + proof_tail_len) * 8

        all_pk_positions = self._roi_positions(
            pixel_array, roi_mask,
            pk_x0, pk_y0, proof_key_int,
            _positions_needed(total_proof_bits),
            sort_by_entropy=False,
            exclude=None,
        )
        tail_positions = all_pk_positions[HEADER_POSITIONS:]
        ...
        # Rebuild proof_key position set for exclusion
        proof_pos_set: Set[Tuple[int, int]] = set(map(tuple, all_pk_positions))

        # ------------------------------------------------------------------
        # Step 4: read metadata from chaos_key positions
        # ------------------------------------------------------------------
        chaos_key_int = generate_chaos_key_from_secret(chaos_key)
        meta_x0, meta_y0 = self._key_start(chaos_key_int, width, height, roi_mask)
        ...
        meta_positions = self._roi_positions(
            pixel_array, roi_mask,
            meta_x0, meta_y0, chaos_key_int,
            _positions_needed(header["metadata_bit_count"]),
            sort_by_entropy=True,
            exclude=proof_pos_set,
        )
...
def _build_proof_block(
    ...
    header = struct.pack(
        _HEADER_FMT,
        MAGIC,                        # 4 bytes
        b"\x01\x00\x00\x00",         # 4 bytes  version=1, padding
