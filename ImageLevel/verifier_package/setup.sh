...
if [ "$ERRORS" -eq 0 ]; then
    ...
    echo ""
    echo "  Public-auditor ZK verification (no key needed):"
    echo "    ./run.sh <stego_image.png> --verify-only"
    echo ""
    echo "  Authorized recipient (full extraction + RDH restore):"
    echo "    ./run.sh <stego_image.png>"
    echo "    # (reads chaos_key.txt automatically)"
    echo ""
    echo "  Or invoke directly:"
    echo "    python3 scripts/dicom_extract.py <stego_image.png> --verify-only"
    echo "    python3 scripts/dicom_extract.py <stego_image.png> --key-file chaos_key.txt"
else
